...
const restaurant = {
  ...
  orederPasta: function (ing1, ing2, ing3) {
    console.log(
      `Here is your declaration pasta with ${ing1}, ${ing2}, and ${ing3}`
    );
  },
...
for (const row of rows) {
  ...
  const fromMod = from.slice(0, 3).toUpperCase();
  const toMod = to.slice(0, 3).toUpperCase();
  const timeMod = `(${time.replace(':', 'h')})`;
  const output = `${firstMod} from ${fromMod} to ${toMod} ${timeMod}`.padStart(
    45,
    ' '
  );
  console.log(output);
...
document.querySelector('button').addEventListener('click', function () {
  ...
  const rowLower = text.toLowerCase().split(/\n/);
  let i = 0;
  for (const str of rowLower) {
    i++;
    const modStrToArr = str.trim().split('_');
    const camelCase =
      modStrToArr[0] +
      modStrToArr[1].replace(
        modStrToArr[1][0],
        modStrToArr[1][0].toUpperCase()
      );
    const camelCaseWiuthPad = camelCase.padEnd(20, ' ');
    const icon = '✅'.repeat(i);
    const outputs = camelCaseWiuthPad + icon;
    console.log(outputs);
  }
});
...
console.log('Maxim Suprunenko'.split(''));

const [firstName, lastName] = 'Maxim Suprunenko'.split(' ');
...
const ingrediants = [
  // prompt("Let's make pasts! Ingrediant 1?"),
  // prompt('Ingrediant 2?'),
  // prompt('Ingrediant 3'),
];
console.log(ingrediants);
restaurant.orederPasta(...ingrediants);

//Objects
const newRestaurant = { foundedIn: 1998, ...restaurant, founder: 'Guis' };
console.log(newRestaurant);

const restaurantCopy = { ...restaurant };
restaurant.name = 'Reistorante Roma';
...
const { name, openingHours, categoWries } = restaurant;
